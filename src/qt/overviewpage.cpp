...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeNumusAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeNumusAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeNumusAmount = strAnonymizeNumusAmount.remove(strAnonymizeNumusAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNumusAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeNumusAmount*COIN) nMaxToAnonymize = nAnonymizeNumusAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeNumusAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeNumusAmount));
        strAnonymizeNumusAmount = strAnonymizeNumusAmount.remove(strAnonymizeNumusAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNumusAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
